...
export default function MyPicksHistoryStraightItem({style, awayTeam, homeTeam, betType, teamPicked, odds, risk, win, gameDate,status}) {
    return (
        <View style={[styles.container, style]}>
            ...
            <View style={[{width:Size.WIDTH*0.95,justifyContent: 'space-between',flexDirection: 'row',paddingHorizontal:20}]}>
                <Text style={{fontFamily: 'Exo2-Regular',color:Color.MAIN}}>
                    {status==='refunded'?'Game cancelled': `Played on ${gameDate}`}
                </Text>
                <Text style={{fontFamily: 'Exo2-Bold',color:status==='loss'? Color.ERROR: Color.HECTIC}}>{status.toUpperCase()}</Text>
            </View>
        </View>
    )
}
